...
/*
    중첩클래스
     ...
     로컬멤버클래스는 메서드가 실행될 때만 사용할 수 있고, 
     메서드가 종료되면 메모리에서 삭제가 된다. 
     
     중첩클래스들도 하나의 클래스이기 때문에 컴파일을 하면 바이트코드파일(--.class)이 
     각각 생성된다. 다만 바이트코드파일의 이름은 "A$B.class"의 형태로 생성된다.
     로컬멤버클래스일 경우에는 "A$1D.class"의 형태로 생성된다.
     
     중첩클래스 중에 정적멤버클래스(C)는 static 키워드로 선언된 중첩클래스를 말한다.
     정적멤버클래스는 모든 종류의 필드와 메서드를 선언할 수 있다.
     
     중첩클래스 중에 로컬멤버클래스는 메서드 내부에 선언된 중첩클래스로서 
     로컬중첩클래스는 접근제한자(public, private), static으로 선언할 수 없다. 
     그 이유는 로컬클래스는 메서드 내에서만 사용되기 때문에 접근을 제한할 필요가 
     없기 때문이다. 로컬중첩클래스에는 인스턴스필드와 인스턴스메서드만 
     선언이 가능하고, static키워드를 사용할 수 없다.
...
//		A.B b = new A.B();  // (x) 객체멤버로 접근불가
		...
		// 2. 정적중첩클래스 (C)
//		A.C.c_field1 = "접근불가";  // (x) c_field1은 객체멤버이기 때문에 C객체 생성없이 접근불가
		A.C.c_field2 = "접근가능";  // (o) 정적멤버이기 때문에 접근가능
//		A.C.c_method1();  // (x) 접근불가 c_method1은 객체멤버이기 때문에 C객체 생성없이 접근불가
		A.C.c_method2();  // (o) 정적멤버이기 때문에 접근가능
		System.out.println();
		
		A.C c = new A.C();  // C는 정적클래스이기 때문에 직접 접근가능
//		A.C c1 = a.new C();  // (x) 접근불가
		c.c_field1 = "객체 생성 후 접근가능";
