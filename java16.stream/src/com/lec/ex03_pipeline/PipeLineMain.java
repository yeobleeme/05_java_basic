...
/*
    ...
    데이터의 합계, 평균, 카운팅, 최대(최소)값 등이 대표적인 reduction의 결과이다.
    그러나 컬렉션의 요소를 리덕션의 결과물로 바로 집계할 수 없을 경우에는 집계하기 편리하도록
    필터링, 매핑, 그룹핑 등의 중간처리를 하는 것이 필요하다.
    
    1. 중간처리와 최종처리
    
       스트림은 데이터의 필터링, 매핑, 정렬, 그룹핑 등의 중간처리와
       합계, 평균, 카운팅 등의 최종처리를 파이프라인(Pipe Line)으로 해결한다.
       파이프라인은 여러개의 스트림이 연결되어 있는 구조를 말한다.
       
       중간스트림이 생성될 때 요소들이 바로 중간처리되는 것이 아니라 최종처리가 
       시작되기 전까지는 중간처리가 지연(lazy)된다. 최종처리가 시작되면 비로소 컬렉션의 요소가
       중간 스트림에서 처리되고 최종처리까지 처리하게 된다.
    ...
    2. 중간처리와 최종처리 메서드
      ...
      2) 최종처리메서드
        ...
        a. 매칭 : allMatch(), anyMatch(), neneMatch()
...
public class PipeLineMain {
	...
	public static void main(String[] args) {
		...
		OptionalDouble avgAge = members.stream()
				                       .filter(m -> m.getGender() == Member.MALE)  // 남자회원만 stream으로 전달
				                       .mapToInt(Member :: getAge)  // Member객체(홍길동, 30) -> stream(30) 전달
				                       .average();  // stream(30, 28, 35) -> 평균(OptinalDouble)
		
		System.out.println("남자회원의 평균나이: " + avgAge.getAsDouble());
		System.out.println();
		
		double avg = members.stream().filter(m -> m.getGender() == Member.FEMALE)
