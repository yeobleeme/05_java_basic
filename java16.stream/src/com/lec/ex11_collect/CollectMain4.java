...
public class CollectMain4 {
	...
	public static void main(String[] args) {
		...
		// 1. 성별로 평균점수 구하기(1) - reducing 과정
		Stream<Student> stream_1 = list.stream();  // 1. stream 생성
		Function<Student, Student.Gender> stream_2 = Student::getGender;  // 2. 성별 분류
		ToDoubleFunction<Student> stream_3 = Student::getScore;  // 3. mapping 처리
		Collector<Student, ?, Double> stream_4 = Collectors.averagingDouble(stream_3);  // 4. 평균 구하기
		...
		System.out.println("남학생 평균: " + by_gender.get(Student.Gender.MALE));
		System.out.println("여학생 평균: " + by_gender.get(Student.Gender.FEMALE));
		System.out.println();
		
		
		// 2. 성별로 평균점수 구하기(2) - reducing 요약
		Map<Student.Gender, Double> by_gender1 = list.stream()
				                                     .collect(Collectors
				                                     .groupingBy(Student::getGender, Collectors.averagingDouble(Student::getScore)));
		System.out.println("남학생 평균: " + by_gender.get(Student.Gender.MALE));
		System.out.println("여학생 평균: " + by_gender.get(Student.Gender.FEMALE));
		System.out.println();
		
		
		// 3. 성별로 그룹핑해서 학생이름을 특정문자(,)로 join
