...
//		List<Integer> list = Arrays.asList(1,2,3,4,5);
		...
		// try-catch 이 외의 예외처리 방법(1) : isPresent(), 요소 존재 여부를 리턴
		...
		// try-catch 이 외의 예외처리 방법(2) : orElse(), 요소가 없을 경우 default값 처리
		double result = list.stream().mapToInt(Integer :: valueOf).average().orElse(1000);
		System.out.println("orElse의 기본값 처리 : " + result);
		
		
		// try-catch 이 외의 예외처리 방법(3) : ifPresent()
		list.stream()
		    .mapToInt(Integer :: intValue)
		    .average()
		    .ifPresent(a -> System.out.println("isPresent : 평균값: " + a));  // 값이 있으면 처리O, 없으면 처리X
