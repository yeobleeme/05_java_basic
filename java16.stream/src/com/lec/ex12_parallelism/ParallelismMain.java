...
/*   
    ...
    멀티코어 CPU환경에서 하나의 작업을 분할해서 각각의 코어가 병렬적으로 처리하는 것을 말한다.
    병렬처리의 목적은 작업시간을 줄이기 위한 것이다.
    Java8버전부터 요소를 병렬처리할 수 있도록 하기 위해 병렬처리스트림을 제공하기 때문에
    컬렉션, 배열의 전체요소를 처리하는 시간을 줄여준다.
    
    동시성:Concurrency 과 병렬성:Parallelism
    
    멀티쓰레드는 동시성 또는 병렬성으로 실행되기 때문에 이 들이 멀티쓰레드의 동작방식이라는
    점에서는 동일하나 각각 다른 목적을 가지고 있다.
    
    동시성은 멀티작업을 위해 멀티쓰레드가 번갈아 가며 실행하는 것을 말하고, 
    병합성은 멀티작업을 위해 멀티코어를 이용해서 동시에 실행하는 것을 말한다.
*/
