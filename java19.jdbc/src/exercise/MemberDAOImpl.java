...
//	@Override
	public void createMember() {
		...
		try {
			...
			System.out.println(row + "건의 정보가 성공적으로 등록되었습니다.");
...
//	@Override
	public ArrayList<MemberVO> findByEmailMember(String memebr_email) {
    ArrayList<MemberVO> memberList = new ArrayList<>();
		...
		ResultSet rs = null;
		String sql = cf.getSelect() 
				   + " where member_name like '%" + memebr_email + "%'";
		...
		try {	
			...
			System.out.println("회원정보 (by 이메일) 조회 실패");
			e.printStackTrace();
		} finally {
			try {
				if(rs != null) rs.close();
