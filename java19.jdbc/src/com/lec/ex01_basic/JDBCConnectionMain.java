...
/*
    ...
    5. CallableStatement 인터페이스
      ...
      CallableStatement는 데이터베이스에 저장된 Stored Procedure를 호출하는 것 만으로 실행이 가능하다.
    ...
    6. ResultSet 인터페이스
      ...
      SQL문 중에서 select문을 사용한 쿼리가 성공했을 경우에 그 결과로서 ResultSet을 반환한다.
      ResultSet은 SQL질의에 의해 생성된 테이블을 담고 있다. 또한, ResultSet객체는 커서(cursor)라고
      하는 것을 가지고 있는데  그 것으로 ResultSet에서 특정값에 대한 참조를 처리할 수 있다.
      
      커서는 초기에 첫 번째 행의 바로 앞을 가리키도록 되어 있는데 ResultSet객체의 nest()메서드를
      이용해서 다음 행으로 이동할 수 있다.
      
      ResultSet의 메서드
      1. first() : 커서를 첫 번쨰 행으로 이동
      2. last() : 커서를 마지막 행으로 이동
      3. beforeFirst() : 커서를 첫 번쨰 행 이전으로 이동
      4. afterLast() : 커서를 마지막 행 이후로 이동
      5. next() : 커서를 다음 행으로 이동
      6. previous() : 커서를 이전 행으로 이동
      
      ResultSet에서 행을 처리하는데 반복문을 사용하여 next()메서드가
      유효한 행이 있으면 true, 없으면 false를 리턴하는 것을 이용해
      while문으로 제어할 수 있다. ResultSet객체에서 값을 가져올 경우에 getXXX()메서드를 사용한다.
      
      ResultSet이 제공하는 메서드
      getString, getInt, getLong, getDouble, getFloat, getDate, getTimestamp, getObject, getBytes
    
    7. JDBC 트랜젝션 
    
      Transaction은 여려 단계의 자ㅏㄱ업을 하나로 처리하는 것으로 하나의 작업으로 인식된 작업이
      모두 성공적으로 종료가 되면 commit이 되고 하나라도 문제가 있다면 rollback이 되어서 작업수행 전으로
      되돌린다. 때문에 Transaction은 프로그램의 신뢰도를 보장하게 된다.
      
      JDBC에서도 Transaction 처리에 대한 메서드를 제공한다.
      
      1. commit : Transaction의 commit을 수행
      2. rollback : Transaction의 rollback을 수행
      
      기본적으로 Connection객체에 setAutoCommit(boolean autoCommit)이란 메서드가 있는데
      기본값이 true로 설정되어 있다. 그렇기 때문에 JDBC는 commit이 자동으로 수행되는데
