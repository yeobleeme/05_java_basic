...
/*
    ...
    자바5부터 제네릭타입이 새롭게 추가되었다. 제네릭타입을 사용하게 되면 
    잘못된 타입이 사용되는 가능성을 컴파일단계부터 제거할 수 있게 되었다.
    제네릭은 컬렉션, 람다식, 스트림, NIO: New Input Output 에 널리 사용되기 때문에
    제네릭을 이해하지 못한다면 자바의 API문서 등을 이해할 수 없다. 
    
     제네릭은 클래스와 인터페이스, 메서드를 정의할 때 타입을 매개값으로 사용할 수 있도록 한다.
     타입파라미터는 코드 작성시에 구체적인 타입으로 선언되어서
     다양한 객체들을 생성할 수 있도록 한다.
     
     제네릭을 사용하는 이점은
     
     1. 컴파일시에 강한 타입체크를 할 수 있다.
        -자바 컴파일러는 코드에서 잘못 사용되는 타입 때문에 발생하는 문제들을
         사전에 제거하기 위해 제네릭코드에서 강한 타입체크를 한다.
         실행시에 타입에러가 발생하는 것 보다 컴파일시에 미리 타입을 
         강하게 제한해서 에러를 미연에 방지할 수 있다.
         
     2. 타입변환을 제거한다.
       -비제네릭코드는 불필요한 타입변환을 하기 때문에 프로그램 성능에 악영향을
        미치게 된다. 제네릭을 사용하면 사전에 정해진 타입으로만 제한 시키기 때문에
        불필요한 타입변환을 할 필요가 없어 프로그램의 성능이 향상된다.
     ...
     제네릭타입은 타입을 파라미터로 갖는 클래스와 인터페이스를 말한다.
     제네릭타입은 클래스 또는 인터페이스이름 뒤에 다이아몬드부호<> 를 붙이고
     그 안에 타입파라미터가 위치한다. 타입파라미터는 변수명과 동일한 규칙에 따라
...
//		float flt = (float) product.getKind();
		...
		Product<Robot, Car> prod4 = new Product<>();
		Product<String, Car> prod5 = new Product<>();
		// Integer, Array ... 참조타입만 가능
