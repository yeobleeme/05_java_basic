...
public class StringMain {
	...
	public static void main(String[] args) {
		...
		// String(문자열)은 참조(Reference)타입, 문자열은 String이라는 Class 타입이다.
		// 자바에서느 클래스로 선언된 것은 모두 참조타입이다.
		// 자바에서 문자열을 표현할 때는 큰 따옴표로 선언한다.
		// 작은 따옴표는 Char타입을 선언할 때 사용한다.
		
		char c1 = 'A';  // 기본타입인 정수타입 중 하나인 char타입
//		char c2 = "A";  // 에러 : char타입은 한 개의 문자(내부적으로 유니코드 정수값만 저장할 수 있다.
		String str1 = "A";  
		String str2 = "AAA";
		...
		// Java에서는 상수의 변수명은 관례로 모두 대문자로 선언, 단어마다 언더바_ 즉, 스케이크방식으로 선언한다.
		// 상수는 final로 선언된 변수를 의미한다. constant값이 한번 설정이 되면 변경할 수 없다.
		final int min_value = 0;  // 관례에 위배
		final int MIN_VALUE = 0;  // 관례에 따른 상수의 변수명 선언
